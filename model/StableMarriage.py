...
class MarriageModel:
        ...
        if singles != []:
            ...
            for person in singles:
                ...
                # if person is in neither of the preference profiles, then print an error message and escape
                    ...
                    raise ValueError(
                        '{} in the matching is not present on any preference lists.'.format(person))
                ...
                blocking_pair = self.__is_blocking_pair(
                    person, would_rather_match, pref_lists, marriages)
        ...
        for spouse1, spouse2 in married.items():
            ...
            if spouse1 in Preferences[0]:
                ...
                if spouse2 in Preferences[0][spouse1]:
                    ...
                    better_than_partner = Preferences[0][spouse1][: Preferences[0][spouse1].index(
                        spouse2)].copy()
                ...
                # look for a blocking pair,
                blocking_pair = self.__is_blocking_pair(
                    spouse1, better_than_partner, Preferences[1], married_reverse)
            ...
            # if spouse1 is neither proposer nor a receiver, print an error message and end the check
                ...
                raise ValueError(
                    '{} in the matching is on a preference lists of the proposing side when they should be on the receiving side.'.format(spouse1))
    ...
    def is_stable(self, mu, preferences=None, married_and_singles_lists=None):
        ...
        # if mu is not a matching,
            ...
            polygamous = next(woman for woman in mu.values()
                              if list(mu.values()).count(woman) > 1)
    ...
    def random_path_to_stability(self, number_of_matchings=1, **kwargs):
        ...
        # will use the preference lists passed at initialization
        preferences = [self.proposers, self.receivers]
        ...
        for i in range(number_of_matchings):
            ...
            mu = {person: None for person in list(
                preferences[0])+list(preferences[1])}
            ...
            while True:
                ...
                blocking_pair = self.is_stable(
                    mu, preferences, (married, singles))
                ...
                if blocking_pair is True:
                    ...
                    if kwargs.get('print_rounds') is True:
                        print(
                            'The algorithm ran {} rounds to reach the following stable matching:'.format(rounds))
                        print(mu)
                    break
                # if it returns a blocking pair
                    ...
                    jilted_proposer = next(
                        (p for p, r in mu.items() if r is blocking_pair[1]), None)
        ...
        self.support, self.frequencies = np.unique(
            output, axis=0, return_counts=True)

        self.support = [{match[0]:match[1] for match in matching}
                        for matching in self.support]
